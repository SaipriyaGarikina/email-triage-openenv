...
IMPORTANT: All scores are STRICTLY within (0, 1) — never 0.0 or 1.0.
  _MIN_SCORE = 0.01  (wrong answer / penalty sentinel)
  _MAX_SCORE = 0.99  (correct answer sentinel)
"""
...
def _score_priority(predicted: Priority, expected: Priority) -> float:
    ...
    Returns:
      0.99 — exact match
      0.49 — one level off  (partial credit)
...
def grade_full_episode(
    ...
    total_score is strictly within (0, 1).
    """
    task = TASK_REGISTRY.get(task_id)
    if task is None:
        raise ValueError(f"Unknown task_id: {task_id}")
